...
import EmployeeProductivityScoreBreakdownTab from './EmployeeProductivityScoreBreakdownTab.jsx';
import ProductivityScoreExportButtons from './ProductivityScoreExportButtons.jsx';
import { CAT_LABELS_SHORT } from '../lib/productivityScoreDisplay.jsx';
...
export default function EmployeeProductivityScoreSection({ onError }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <InfoHint
            ...
            text="Scores aggregate Command Centre team members only. Use Score breakdown to see every event that contributed to each person's total. Grace credits and debtor sanctions are issued by team leaders from team pools under Warnings & rewards."
          />
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <ProductivityScoreExportButtons data={data} disabled={loading} onError={onError} />
        </div>
      </div>
      ...
      {subTab === 'breakdown' && (
        <EmployeeProductivityScoreBreakdownTab
          data={data}
          loading={loading}
          onReload={load}
      ...
      {subTab === 'overview' && (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-xl border border-surface-200 bg-white p-4 shadow-sm">
              ...
              <div className="text-xs text-surface-700 mt-2 space-y-0.5 tabular-nums">
                {Object.entries(CAT_LABELS_SHORT).map(([k, label]) => (
                  <div key={k}>
                    {label}: {data?.componentAverages?.[k] ?? 0}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="rounded-xl border border-surface-200 bg-white overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-surface-100 bg-surface-50/80 flex flex-wrap items-center justify-between gap-2">
              <div>
              ...
              <table className="w-full text-sm min-w-[720px]">
                <thead className="bg-surface-50 border-b border-surface-200">
                  <tr>
                    ...
                    {Object.keys(CAT_LABELS_SHORT).map((k) => (
