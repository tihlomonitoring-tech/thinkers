...
export default function Rector() {
  ...
  useEffect(() => {
    if (!hasTenant || activeTab !== 'targets-regulations') return;
    let cancelled = false;
    contractorApi.routeTargetRegulations.list()
      .then((r) => { if (!cancelled) setTargetRegulations(r.regulations || []); })
      .catch(() => { if (!cancelled) setTargetRegulations([]); });
    return () => { cancelled = true; };
  }, [hasTenant, activeTab]);
