...
/** Company library: uploader receives code to share with requester for download. @deprecated — use companyLibrarySystemPinToUploaderHtml */
export function companyLibraryDownloadCodeToUploaderHtml({
  uploaderName,
  requesterName,
  requesterEmail,
  documentTitle,
  ...
  return companyLibrarySystemPinToUploaderHtml({
    uploaderName,
    requesterName,
    requesterEmail,
    documentTitle,
...
export function companyLibrarySystemPinToUploaderHtml({
  ...
  const inner = `
    ...
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;"><strong>${escapeHtml(requesterName || 'A colleague')}</strong>${requesterEmail ? ` (${escapeHtml(requesterEmail)})` : ''} is requesting a copy of a <strong>secured</strong> document you uploaded in the <strong>Company library</strong>. Files are delivered only by email — there is no browser download.</p>
    <p style="margin: 0 0 8px 0; font-size: 14px; color: #475569;"><strong>Document:</strong> ${escapeHtml(documentTitle || '—')}</p>
    <p style="margin: 0 0 16px 0; font-size: 14px; color: #475569;">The PIN below was generated by the system (you did not set it). Share it with the requester only if you approve. They will enter it in the app; then the file will be emailed to <em>their</em> address.</p>
    <div style="background: #ecfdf5; border: 1px solid #6ee7b7; border-radius: 10px; padding: 16px 20px; text-align: center; margin: 20px 0;">
      <p style="margin: 0 0 6px 0; font-size: 12px; font-weight: 600; color: #047857; text-transform: uppercase; letter-spacing: 0.06em;">System PIN — share with requester if you approve</p>
      <p style="margin: 0; font-size: 28px; font-weight: 700; letter-spacing: 0.15em; color: #065f46; font-family: ui-monospace, monospace;">${escapeHtml(String(pin || ''))}</p>
      <p style="margin: 10px 0 0 0; font-size: 12px; color: #047857;">Expires in about ${escapeHtml(String(expiresMinutes))} minutes.</p>
    </div>
    <p style="margin: 0; font-size: 13px; color: #64748b;">If you did not expect this request, ignore this email — the requester cannot receive the file without this PIN.</p>
    ${appUrl ? `<p style="margin: 16px 0 0 0;"><a href="${escapeHtml(appUrl)}" style="color:#2563eb;">Open Company library</a></p>` : ''}
  `;
  return taskEmailLayout('Company library — system PIN for secured document', inner, 'Company library');
}

/** Secured library doc: super admin receives system PIN on their own email (no uploader handoff). */
export function companyLibrarySystemPinToSuperAdminHtml({
  adminName,
  documentTitle,
  pin,
  appUrl,
  ...
  const inner = `
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">Hello ${escapeHtml(adminName || 'there')},</p>
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">You requested access to a <strong>secured</strong> document in the <strong>Company library</strong>. As super admin, this system-generated PIN was sent to you directly — you do not need the uploader to share it.</p>
    <p style="margin: 0 0 8px 0; font-size: 14px; color: #475569;"><strong>Document:</strong> ${escapeHtml(documentTitle || '—')}</p>
    <p style="margin: 0 0 16px 0; font-size: 14px; color: #475569;">Enter the PIN in the app to unlock repeated “email copy to me” actions until the session expires. The file is never available via a direct download link.</p>
    <div style="background: #eff6ff; border: 1px solid #93c5fd; border-radius: 10px; padding: 16px 20px; text-align: center; margin: 20px 0;">
      <p style="margin: 0 0 6px 0; font-size: 12px; font-weight: 600; color: #1e40af; text-transform: uppercase; letter-spacing: 0.06em;">Your system PIN</p>
      <p style="margin: 0; font-size: 28px; font-weight: 700; letter-spacing: 0.15em; color: #1e3a8a; font-family: ui-monospace, monospace;">${escapeHtml(String(pin || ''))}</p>
      <p style="margin: 10px 0 0 0; font-size: 12px; color: #1d4ed8;">Expires in about ${escapeHtml(String(expiresMinutes))} minutes.</p>
    </div>
    <p style="margin: 0; font-size: 13px; color: #64748b;">Do not forward this PIN unless you intend another person to receive the file by email from their own account.</p>
    ${appUrl ? `<p style="margin: 16px 0 0 0;"><a href="${escapeHtml(appUrl)}" style="color:#2563eb;">Open Company library</a></p>` : ''}
  `;
  return taskEmailLayout('Company library — super admin PIN', inner, 'Company library');
}

/** Library file attached — sent only to the requester’s email. */
export function companyLibraryAttachmentEmailedHtml({
  recipientName,
  documentTitle,
  fileName,
  secured,
  ...
  const inner = `
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">Hello ${escapeHtml(recipientName || 'there')},</p>
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">As requested, your copy of the company library document is <strong>attached</strong> to this message.</p>
    ${keyValueTable([
      ['Document', documentTitle || '—'],
      ['Attachment name', fileName || '—'],
      ['Secured workflow', secured ? 'Yes (PIN was verified)' : 'No (open library document)'],
    ])}
    ${note ? `<p style="margin: 16px 0 0 0; font-size: 13px; color: #64748b;">${escapeHtml(note)}</p>` : ''}
    <p style="margin: 16px 0 0 0; font-size: 13px; color: #64748b;">Direct download links are disabled for library files; keep this email if you need the attachment.</p>
  `;
  return taskEmailLayout('Company library — your document copy', inner, 'Company library');
}

/** Super admin: one-time code emailed to authorize permanent removal of a library file. */
export function companyLibrarySuperAdminDeleteCodeHtml({ adminName, documentTitle, code, expiresMinutes, appUrl }) {
  const inner = `
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">Hello ${escapeHtml(adminName || 'there')},</p>
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">You (or someone signed in as you) requested authorization to <strong>permanently delete</strong> a file from the <strong>Company library</strong>.</p>
    <p style="margin: 0 0 8px 0; font-size: 14px; color: #475569;"><strong>Document:</strong> ${escapeHtml(documentTitle || '—')}</p>
    <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 10px; padding: 16px 20px; text-align: center; margin: 20px 0;">
      <p style="margin: 0 0 6px 0; font-size: 12px; font-weight: 600; color: #991b1b; text-transform: uppercase; letter-spacing: 0.06em;">Delete authorization code</p>
      <p style="margin: 0; font-size: 28px; font-weight: 700; letter-spacing: 0.15em; color: #7f1d1d; font-family: ui-monospace, monospace;">${escapeHtml(String(code || ''))}</p>
      <p style="margin: 10px 0 0 0; font-size: 12px; color: #b91c1c;">Enter this code in the app to confirm deletion. Expires in about ${expiresMinutes || 15} minutes. Do not share this code.</p>
    </div>
    <p style="margin: 0; font-size: 13px; color: #64748b;">If you did not request this, ignore this email — the file will not be deleted without the code.</p>
    ${appUrl ? `<p style="margin: 16px 0 0 0;"><a href="${escapeHtml(appUrl)}" style="color:#2563eb;">Open Company library</a></p>` : ''}
  `;
  return taskEmailLayout('Company library — delete authorization', inner, 'Company library');
}

export function companyLibraryExpiryReminderHtml({ recipientName, documentTitle, expiresAt, appUrl }) {
  const inner = `
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">Hello ${escapeHtml(recipientName || 'there')},</p>
    <p style="margin: 0 0 12px 0; font-size: 15px; color: #334155;">This is a reminder that a document in the <strong>Company library</strong> is approaching its review / expiry date.</p>
    <p style="margin: 0 0 8px 0; font-size: 14px; color: #475569;"><strong>Document:</strong> ${escapeHtml(documentTitle || '—')}</p>
    <p style="margin: 0 0 16px 0; font-size: 14px; color: #475569;"><strong>Expiry / review date:</strong> ${escapeHtml(expiresAt || '—')}</p>
    <p style="margin: 0; font-size: 13px; color: #64748b;">Please renew or update the record as required by your organisation’s policy.</p>
    ${appUrl ? `<p style="margin: 16px 0 0 0;"><a href="${escapeHtml(appUrl)}" style="color:#2563eb;">Open Company library</a></p>` : ''}
  `;
