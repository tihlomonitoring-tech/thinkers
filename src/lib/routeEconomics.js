/** Route corridor economics and Route Smart Outlook (RSO) calculations. */
...
export function computeRouteEconomics(input = {}) {
  ...
  const revenuePerKm = distanceKm > 0 ? round2(revenuePerTrip / distanceKm) : null;
  const costPerKm = distanceKm > 0 ? round2(totalCostPerTrip / distanceKm) : null;
  const breakEvenRatePerTon = avgPayload > 0 ? round2(totalCostPerTrip / avgPayload) : null;

  const tripsPerTruckPeriod = deliveriesPerTruck;
  const totalTripsPeriod = round2(tripsPerTruckPeriod * enrolledTrucks);
  ...
  const tripsNeededForTarget = revenuePerTrip > 0 && revenueTarget > 0
    ? Math.ceil(revenueTarget / revenuePerTrip)
    : null;
  const extraTripsNeeded = tripsNeededForTarget != null
    ? Math.max(0, tripsNeededForTarget - totalTripsPeriod)
    : null;
  const extraDeliveriesPerTruck = enrolledTrucks > 0 && extraTripsNeeded > 0
    ? round2(extraTripsNeeded / enrolledTrucks)
    : 0;
  ...
  if (costPerTon != null && ratePerTon > 0) {
    insights.push({
      level: 'info',
      text: `All-in cost ${fmtMoney(costPerTon)}/t vs rate R${ratePerTon}/t (${distanceKm} km corridor).`,
    });
  }
