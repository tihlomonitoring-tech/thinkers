/** Professional route corridor risk assessment scoring. */
...
export function parseRiskAssessmentJson(raw) {
  if (!raw) return null;
  try {
    const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
    if (!parsed || typeof parsed !== 'object') return null;
    return { ...defaultRiskAssessment(), ...parsed, scores: { ...defaultRiskAssessment().scores, ...(parsed.scores || {}) } };
  } catch {
    return null;
  }
}
